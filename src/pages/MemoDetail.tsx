    ...
    <div className="max-w-5xl mx-auto pb-12">
      ...
      <Card className="mb-6 overflow-hidden border border-ink-200 shadow-sm">
        ...
        <CardBody className="p-6">
          {/* Document Body with approval stamp overlay */}
          <div className="relative">
            ...
            {/* ── OSTA Approval Stamp ──────────────────────────────────────
                Shown only when memo has been Approved or Completed.
                Positioned bottom-right, rotated slightly like a real ink stamp,
                and semi-transparent so the document text shows through. */}
            {(memo.status === 'Approved' || memo.status === 'Completed') && (
              <div
                className="pointer-events-none absolute bottom-3 right-4 select-none"
                aria-label="OSTA Official Approval Stamp"
              >
                <img
                  src={ostaStamp}
                  alt="Oromia Science and Technology Authority — Approved"
                  className="w-36 h-36 object-contain"
                  style={{ opacity: 0.55, transform: 'rotate(-12deg)' }}
                />
                <p className="text-center text-[9px] font-bold text-brand-700 mt-1 tracking-wider uppercase opacity-70">
                  Officially Approved
                </p>
              </div>
            )}
                ...
                {memo.attachments.map((a, i) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <div className="size-8 rounded bg-brand-100 text-brand-700 flex items-center justify-center shrink-0 font-bold text-[10px] uppercase">
                        {a.type || 'DOC'}
                      </div>
                      <div className="min-w-0">
                        <p className="text-xs font-semibold text-ink-900 truncate">{a.name}</p>
                        <p className="text-[10px] text-ink-400">{a.size}</p>
                      </div>
                    </div>
                    <button
                      onClick={() => dispatch(pushToast(`Opening attachment: ${a.name}`, 'info'))}
                      className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium rounded text-brand-700 hover:bg-brand-100 bg-white border border-brand-200 shrink-0"
                    >
