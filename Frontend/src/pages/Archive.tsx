...
const ALL_TYPES = Object.keys(memoTypeKeys);
const ALL_STATUSES = Object.keys(statusKeys);

export default function Archive() {
  const { t } = useTranslation();
  const dispatch = useAppDispatch();
  const archiveDocs = useAppSelector((s) => s.archive.items);
  const pagination = useAppSelector((s) => s.archive.pagination);
  ...
  // Debounce keyword search a little so we don't hit the API on every keystroke.
  useEffect(() => {
  ...
  }, [dispatch, page, type, department, status, keyword]);

  // Any filter change should jump back to page 1.
  useEffect(() => { setPage(1); }, [type, department, status, keyword]);

  const filtered = useMemo(() => archiveDocs, [archiveDocs]);
      ...
      <Card className="mb-5">
        <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <select value={type} onChange={(e) => setType(e.target.value)} className="px-3 py-2.5 rounded-lg border border-ink-200 text-sm outline-none bg-white dark:bg-ink-100">
            <option value="All">{t('all_types_label')}</option>
            {ALL_TYPES.map((ty) => <option key={ty} value={ty}>{t(memoTypeKeys[ty])}</option>)}
          </select>
