...
export default function Channels() {
  ...
  return (
    <div>
      <h1 className="text-2xl mb-1">Channels</h1>
      <p className="text-sm text-ink-400 mb-5">Departmental and cross-departmental discussion channels.</p>
      ...
      <div className="bg-white rounded-xl border border-ink-100 shadow-sm overflow-hidden grid grid-cols-1 md:grid-cols-[300px_1fr]" style={{ height: 'calc(100vh - 220px)', minHeight: 480 }}>
        ...
        <div className={clsx(active ? 'flex' : 'hidden md:flex', 'flex-col')}>
          {active && (
            <button onClick={() => setActive(null)} className="md:hidden text-xs text-brand-700 px-4 py-2 border-b border-ink-100 text-left">← Back to channels</button>
          )}
