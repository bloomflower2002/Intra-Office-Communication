...
import { setActiveThread, fetchChannels } from '../features/messages/messagesSlice';
...
export default function Channels() {
  ...
  const channelsPagination = useAppSelector((s) => s.messages.channelsPagination);
  const [active, setActive] = useState<string | null>(null);
  const [page, setPage] = useState(1);

  useEffect(() => {
    dispatch(fetchChannels({ page, pageSize: 20 }));
  }, [dispatch, page]);
        ...
        <div className={clsx('border-r border-ink-100 flex-col overflow-y-auto scrollbar-thin', active ? 'hidden md:flex' : 'flex')}>
          ...
          {channelsPagination && channelsPagination.totalPages > 1 && (
            <div className="px-3 py-2 shrink-0">
              <Pagination pagination={channelsPagination} onPageChange={setPage} />
            </div>
          )}
        </div>
