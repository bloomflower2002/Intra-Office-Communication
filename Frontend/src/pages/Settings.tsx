    ...
    <div className="max-w-3xl">
      ...
      <TabGroup>
        <TabList className="flex gap-1.5 mb-5 border-b border-ink-100">
          {['tab_profile', 'tab_security', 'tab_notifications'].map((tabKey) => (
        ...
        <TabPanels>
          <TabPanel>
            <Card>
              <CardHeader><h3 className="text-sm font-semibold">{t('profile_information')}</h3></CardHeader>
              <CardBody className="space-y-4">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-ink-400">{user.department} · {t(roleKeys[user.role])}</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-ink-700 mb-1.5">{t('full_name')}</label>
                  ...
                  <div>
                    <label className="block text-sm font-medium text-ink-700 mb-1.5">{t('email')}</label>
                ...
                <Button onClick={() => dispatch(pushToast('Profile updated successfully.', 'success'))}>{t('save_changes')}</Button>
              </CardBody>
            </Card>
          </TabPanel>
          ...
          <TabPanel>
            <Card>
              <CardHeader><h3 className="text-sm font-semibold">{t('security')}</h3></CardHeader>
              <CardBody className="space-y-4 max-w-md">
                <div>
                  <label className="block text-sm font-medium text-ink-700 mb-1.5">{t('current_password')}</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-ink-700 mb-1.5">{t('new_password')}</label>
                  <input type="password" placeholder="••••••••" className="w-full px-3 py-2.5 rounded-lg border border-ink-200 text-sm outline-none focus:border-brand-500" />
                </div>
                <Button onClick={() => dispatch(pushToast('Password updated.', 'success'))}>{t('update_password')}</Button>
              </CardBody>
            </Card>
          </TabPanel>
          ...
          <TabPanel>
            <Card>
              <CardHeader><h3 className="text-sm font-semibold">{t('notification_preferences')}</h3></CardHeader>
              <CardBody className="space-y-4">
                <label className="flex items-center justify-between cursor-pointer">
                  <div><p className="text-sm font-medium text-ink-700">{t('email_notifications')}</p><p className="text-xs text-ink-400">{t('receive_memo_and_approval_updates_via_email')}</p></div>
                ...
                <label className="flex items-center justify-between cursor-pointer">
                  <div><p className="text-sm font-medium text-ink-700">{t('push_in_app_notifications')}</p><p className="text-xs text-ink-400">{t('show_notification_badge_and_alerts_in_the_app')}</p></div>
                ...
                <Button onClick={() => dispatch(pushToast('Notification preferences saved.', 'success'))}>{t('save_preferences')}</Button>
              </CardBody>
            </Card>
          </TabPanel>
        </TabPanels>
