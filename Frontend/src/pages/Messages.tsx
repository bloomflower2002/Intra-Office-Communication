...
export default function Messages() {
  ...
  const filtered = threads.filter((t) => {
    const u = getUser(t.participantId);
    return u?.fullName.toLowerCase().includes(query.toLowerCase());
  });

  const activeUser = active ? getUser(threads.find((t) => t.id === active)?.participantId ?? '') : null;

  return (
    <div>
      <h1 className="text-2xl mb-1">Messages</h1>
      <p className="text-sm text-ink-400 mb-5">Direct, one-on-one conversations with your colleagues.</p>

      <div className="bg-white rounded-xl border border-ink-100 shadow-sm overflow-hidden grid grid-cols-1 md:grid-cols-[300px_1fr]" style={{ height: 'calc(100vh - 220px)', minHeight: 480 }}>
          ...
          <div className="p-3 border-b border-ink-100">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-ink-50 rounded-lg pl-9 pr-3 py-2 text-sm outline-none border border-transparent focus:border-brand-300 focus:bg-white"
              />
            </div>
          </div>
        ...
        <div className={clsx(active ? 'flex' : 'hidden md:flex', 'flex-col')}>
          {active && (
            <button onClick={() => setActive(null)} className="md:hidden text-xs text-brand-700 px-4 py-2 border-b border-ink-100 text-left">← Back to contacts</button>
          )}
