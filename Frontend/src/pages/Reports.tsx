...
export default function Reports() {
  ...
  const archivePagination = useAppSelector((s) => s.archive.pagination);
  const departments = useAppSelector((s) => s.departments.items);
  const { departmentVolume, statusBreakdown } = useAppSelector((s) => s.dashboard);
  ...
  useEffect(() => {
    dispatch(fetchArchive({ page, pageSize: 25, department: dept === 'All' ? undefined : dept }));
  }, [dispatch, page, dept]);
  ...
  const handleExport = () => {
    ...
    dispatch(pushToast('Report exported as CSV (current page).', 'success'));
  };
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-5">
        <div>
          <h1 className="text-2xl mb-1">{t('reports_analytics')}</h1>
          <p className="text-sm text-ink-400">{t('organization_wide_communication_and_performance_re')}</p>
        </div>
        <Button icon={<Download className="size-4" />} onClick={handleExport}>{t('export_to_csv')}</Button>
      </div>

      <Card className="mb-5">
        <CardBody className="flex flex-wrap items-end gap-4">
          <div>
            <label className="block text-xs font-medium text-ink-500 mb-1.5">{t('from')}</label>
          ...
          <div>
            <label className="block text-xs font-medium text-ink-500 mb-1.5">{t('to')}</label>
          ...
          <div>
            <label className="block text-xs font-medium text-ink-500 mb-1.5">{t('department')}</label>
            <select value={dept} onChange={(e) => setDept(e.target.value)} className="px-3 py-2 rounded-lg border border-ink-200 text-sm outline-none bg-white dark:bg-ink-100">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-4 mb-5">
        <Card className="xl:col-span-3">
          <CardHeader><h3 className="text-sm font-semibold">{t('volume_by_department')}</h3></CardHeader>
        ...
        <Card className="xl:col-span-2">
          <CardHeader><h3 className="text-sm font-semibold">{t('status_breakdown')}</h3></CardHeader>
      ...
      <Card>
        <CardHeader><h3 className="text-sm font-semibold">{t('report_details')}</h3></CardHeader>
        ...
        {archivePagination && (
          <div className="px-5 py-3 border-t border-ink-100">
            <Pagination pagination={archivePagination} onPageChange={setPage} />
          </div>
        )}
      </Card>
    </div>
