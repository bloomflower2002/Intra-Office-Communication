...
function UserManagement() {
  ...
  const usersPagination = useAppSelector((s) => s.users.pagination);
  ...
  useEffect(() => {
    const handle = setTimeout(() => {
      dispatch(fetchUsers({ page, pageSize: 30, search: query || undefined }));
    }, query ? 300 : 0);
...
const institutionalDefaultRoles: CustomRoleDef[] = [
  {
    role: 'System Admin',
    description: 'Full institutional governance, security policies, user provisioning & audit oversight (Channels, Memos, Approvals, and Archive can be enabled by ticking permissions below)',
    permissions: [
      'Manage Institutional Users & Status',
      'Configure System Policies & Branding',
      'Manage Organizational Hierarchy',
...
function RoleManagement() {
  ...
  const [roleDefs, setRoleDefs] = useState<CustomRoleDef[]>(() => getSavedRoleDefs());
  ...
  const handleSelectAllPerms = (roleName: string) => {
    setRoleDefs((prev) =>
      prev.map((r) => (r.role === roleName ? { ...r, permissions: [...availablePermissions] } : r))
    );
    setHasUnsavedChanges(true);
  };

  const handleClearAllPerms = (roleName: string) => {
    setRoleDefs((prev) =>
  ...
  const handleSaveChanges = () => {
    saveRoleDefs(roleDefs);
    setHasUnsavedChanges(false);
    dispatch(pushToast('Role permissions and security hierarchy successfully saved.', 'success'));
  };

  const handleResetDefaults = () => {
    setRoleDefs(institutionalDefaultRoles);
    saveRoleDefs(institutionalDefaultRoles);
    setHasUnsavedChanges(false);
    dispatch(pushToast('Role hierarchy restored to institutional defaults.', 'info'));
  };
  ...
  const handleAddRole = (e: React.FormEvent) => {
    ...
    setRoleDefs(updated);
    saveRoleDefs(updated);
    setHasUnsavedChanges(false);
    dispatch(pushToast(`Role "${newRoleName.trim()}" created and saved successfully.`, 'success'));
    setNewRoleName('');
    setNewRoleDesc('');
  ...
  return (
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            {hasUnsavedChanges ? (
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-800 border border-amber-200 animate-pulse">
          ...
          <p className="text-xs text-ink-400 mt-0.5">
            Configure institutional security privileges and authorization scopes across operational tiers
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <Button
            ...
            title="Restore institutional defaults"
          >
            Reset Defaults
          </Button>
          <Button
          ...
          <Button
            ...
            className={hasUnsavedChanges ? '!bg-emerald-600 hover:!bg-emerald-700 !text-white shadow-md ring-2 ring-emerald-500/50' : ''}
          >
            Save Changes
          </Button>
        </div>
      ...
      {hasUnsavedChanges && (
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/50 mb-4 shadow-sm">
          <div className="flex items-center gap-2 text-xs text-amber-900 dark:text-amber-200">
            <span className="size-2 rounded-full bg-amber-500 shrink-0" />
            <span>You have modified role permissions. Click <strong>"Save Changes"</strong> to apply your configuration across the system.</span>
          </div>
          <div className="flex items-center gap-2 self-end sm:self-auto">
            <button
              onClick={handleResetDefaults}
              className="text-xs text-ink-600 hover:text-ink-900 px-2 py-1 rounded hover:bg-white/60 transition-colors font-medium"
            >
              Discard
            </button>
            <Button size="sm" icon={<CheckCircle2 className="size-3.5" />} onClick={handleSaveChanges}>
              Save Changes Now
            </Button>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {roleDefs.map((r) => {
          const activeCount = r.permissions.filter((p) => availablePermissions.includes(p)).length;
          return (
            <Card key={r.role} className="flex flex-col justify-between">
              <CardHeader className="flex items-center justify-between pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="text-sm font-bold text-ink-900">{r.role}</h3>
                      {r.isCustom && (
                    ...
                    <span className="text-[11px] text-ink-400 font-mono">
                      {activeCount} / {availablePermissions.length} Permissions Active
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1">
                  {r.isCustom && (
                    <button
                      ...
                      className="text-ink-400 hover:text-danger-500 transition-colors p-1 rounded hover:bg-danger-50"
              ...
              <CardBody className="pt-0 flex-1 flex flex-col justify-between">
                <p className="text-xs text-ink-500 mb-3 leading-relaxed">{r.description}</p>

                {/* Quick Toggle Controls */}
                <div className="flex items-center justify-between py-1.5 px-2 bg-ink-50 dark:bg-ink-200/40 rounded-lg text-[11px] mb-2 text-ink-500">
                  <span>Scope Privileges:</span>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => handleSelectAllPerms(r.role)}
                    ...
                    <span>·</span>
                    <button
                      onClick={() => handleClearAllPerms(r.role)}
                      className="text-ink-400 hover:text-ink-700 font-medium"
                    >
                      Clear All
                    </button>
                  </div>
                ...
                <div className="space-y-1.5 max-h-56 overflow-y-auto scrollbar-thin pr-1 border border-ink-100 dark:border-ink-200 rounded-lg p-2 bg-white dark:bg-ink-100">
                    ...
                    return (
                      <label
                        ...
                        className={`flex items-center justify-between text-xs p-1.5 rounded cursor-pointer transition-colors ${
                          has ? 'bg-brand-50/60 dark:bg-brand-950/30 text-ink-900 font-medium' : 'text-ink-600 hover:bg-ink-50'
                        }`}
                      >
                        <span className="flex items-center gap-2 min-w-0 pr-2">
                          <span
                            className={`size-2 rounded-full shrink-0 ${has ? 'bg-emerald-500 ring-2 ring-emerald-200' : 'bg-ink-300'}`}
                          />
                        ...
                        <input
                          ...
                          className="rounded border-ink-300 text-brand-600 size-4 shrink-0 focus:ring-brand-500"
                        />
                      </label>
                    );
                  })}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button onClick={handleAddRole}>Create & Save Role</Button>
          </>
        }
      >
        <form onSubmit={handleAddRole} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="e.g. Compliance Auditor, Section Chief"
              className="w-full px-3 py-2 rounded-lg border border-ink-200 text-sm outline-none focus:border-brand-500 bg-white dark:bg-ink-100"
              required
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Brief description of institutional responsibilities"
              className="w-full px-3 py-2 rounded-lg border border-ink-200 text-sm outline-none focus:border-brand-500 bg-white dark:bg-ink-100"
            />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-medium text-ink-700">Granted Permissions</label>
            ...
            <div className="space-y-2 border border-ink-200 rounded-lg p-3 max-h-48 overflow-y-auto scrollbar-thin bg-white dark:bg-ink-100">
              {availablePermissions.map((p) => {
                const checked = selectedPerms.includes(p);
                return (
                  <label key={p} className="flex items-center gap-2 text-xs text-ink-700 cursor-pointer hover:bg-ink-50 p-1 rounded">
                    <input
                      type="checkbox"
                      checked={checked}
                      onChange={() => {
