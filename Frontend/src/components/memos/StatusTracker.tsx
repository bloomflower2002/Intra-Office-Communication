        ...
        return (
          <div key={stage} className="flex-1 flex flex-col items-center relative">
            ...
            <div className={clsx(
              ...
              reached ? 'bg-brand-600 border-brand-600 text-white' : 'bg-white border-ink-200 text-ink-300',
            )}>
              {reached ? <Check className="size-4" /> : i + 1}
            </div>
            <p className={clsx('text-xs font-medium mt-2 text-center', reached ? 'text-ink-800' : 'text-ink-400')}>{stage}</p>
